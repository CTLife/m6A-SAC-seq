...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2-mutationNOT0/minus.T.txt", global variable.
my $output_g = '';  ## such as "3-BEDregion/minus.T.bed", global variable.
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        For instance:
               perl  3-BEDregion.pl    -in 2-mutationNOT0/minus.T.txt   -out 3-BEDregion/minus.T.bed    > 3-BEDregion.runLog.txt
...
$input_g  = '2-mutationNOT0/minus.T.txt';         ## This is only an initialization value or suggesting value, not default value.
$output_g = '3-BEDregion/minus.T.bed';            ## This is only an initialization value or suggesting value, not default value.
...
#open(FHout2, ">", "$output_g.filtered1.bed")  or  die;
#open(FHout3, ">", "$output_g.filtered2.bed")  or  die;
#open(FHout4, ">", "$output_g.filtered3.bed")  or  die;

open(refX_tumorA, ">", "$output_g.tumorA.bed")  or  die;
open(refX_tumorC, ">", "$output_g.tumorC.bed")  or  die;
open(refX_tumorG, ">", "$output_g.tumorG.bed")  or  die;
open(refX_tumorT, ">", "$output_g.tumorT.bed")  or  die;
open(refX_tumorN, ">", "$output_g.tumorN.bed")  or  die;
...
for(my $i=1; $i<=$#lines; $i++) {
    $lines[$i] =~ m/^(\S+)\t(\d+)\t(\S+)\t(\S*)\t(\d+)\t(\d+)\t(\S+)\t(\S+)\t(\d+)\t(\d+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\t(\S+)\n$/ or die;
    ...
    if( ($somatic_p_value<0.05) and ($tumor_var_freq>$normal_var_freq)  ) {
    ...
    #if( ($tumor_var_freq>$normal_var_freq)  ) {
    #    print FHout2 "$chrom\t$start\t$end\t$name\t$score\t$strand_g\n";
    #}
    #if( ($somatic_p_value<0.05) and ($tumor_var_freq>$normal_var_freq)  ) {
    #    print FHout3 "$chrom\t$start\t$end\t$name\t$score\t$strand_g\n";
    #}
    #if( ($somatic_p_value<0.05) and ($tumor_var_freq>$normal_var_freq) and (5>$normal_var_freq) ) {
    #    print FHout4 "$chrom\t$start\t$end\t$name\t$score\t$strand_g\n";
    #}
}
