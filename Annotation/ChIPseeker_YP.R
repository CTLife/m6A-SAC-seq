...
## version 0.4.0, February 22, 2020.
## Run "ChIPseeker_YP -h" to get some help.
##############################################################################################################################################################################################
...
MyPeaksDistribution_OneGroup_1_g <- function(myPeak_t1, myPath_t1, up1, down1) { 
  #myTempFunction3 <- function() {    
      if( ! file.exists(myPath_t1)  ) { dir.create(myPath_t1,  recursive = TRUE)  }
      peakAnno <- annotatePeak( myPeak_t1, tssRegion=c(-up1, down1),  TxDb=my_txdb_g, annoDb=my_orgdb_g, sameStrand = stranded_g  )  
...
MyAnnotation_OneGroup_1_g <- function(myFile_t1, myPath_t1) {
  ...
  MyPeaksDistribution_OneGroup_1_g(myPeak_t1 = myPeak_t1, myPath_t1 = paste(myPath_t1,  "4_GenomicRegions-distribution_x2",  sep="/"),  up1=upstream_g*2, down1=downstream_g*2)
  
  peakAnno <- annotatePeak( myPeak_t1, tssRegion=c(-upstream_g, downstream_g),  TxDb=my_txdb_g, annoDb=my_orgdb_g, sameStrand = stranded_g  )  
