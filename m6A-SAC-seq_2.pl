...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1-rawFASTQ"   
my $output_g = '';  ## such as "2-removedAdapters"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 2: Remove adaptors and PCR primers by using Trimmomatic. 
        ...
        Usage:
               m6A-SAC-seq_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               m6A-SAC-seq_2.pl   -in 1-rawFASTQ   -out 2-removedAdapters    > m6A-SAC-seq_2.runLog.txt  2>&1   
...
## Version Infromation
my $version = "    The 2nd Step, version 1.0.0,  2020-03-01.";
...
$output_g = '2-removedAdapters';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'm6A-SAC-seq_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("java  -jar  $Trimmomatic_g  PE   -threads $numCores_g  -summary  $output2_g/$temp.statsSummaryFile.txt   $input_g/$end1  $input_g/$end2    $output_g/$end1  $output_g/unpaired-$end1    $output_g/$end2  $output_g/unpaired-$end2      ILLUMINACLIP:0-Others/Adapters_SAC-seq_list-1.fasta:2:30:10:1:TRUE   MINLEN:15    >> $output2_g/$temp.runLog  2>&1");                                                   
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/$singleEnd_g[$i]    ILLUMINACLIP:0-Others/Adapters_SAC-seq_list-1.fasta:2:30:10   MINLEN:25      >> $output2_g/$temp.runLog.txt  2>&1");                    
}
}
###################################################################################################################################################################################################
...
{
say   "\n\n\n\n\n\n##################################################################################################";
say   "Detecting the quality of paired-end FASTQ files by using fastp with PE mode ......";
my $fastp   = "$output2_g/4_fastp_PairedEnd";    
my $MultiQC = "$output2_g/MultiQC/4_fastp_PairedEnd";
&myMakeDir($fastp); 
&myMakeDir($MultiQC);
...
system( "multiqc  --title fastp    --filename  fastp   --module  fastp   --verbose  --export  --outdir $MultiQC    $fastp   >> $MultiQC/MultiQC.fastp.runLog.txt    2>&1" );
}
###################################################################################################################################################################################################
