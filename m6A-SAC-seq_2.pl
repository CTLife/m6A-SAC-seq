...
my $HELP = '
        ...
        Step 2: Remove adapters and bases with low quality by using Trimmomatic and trim_galore. 
                ...
                The versions of softwares and packages are used in this script (This is shown for reference only. And do not need to exactly match them.): 
                        ...
                        cutadapt,      4.1     (cutadapt      --version)
       
        Usage:
               perl m6A-SAC-seq_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl m6A-SAC-seq_2.pl   -in 1_rawFASTQ   -out 2_finalFASTQ    > m6A-SAC-seq_2.runLog.txt  2>&1    &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl m6A-SAC-seq_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/Trimmomatic/$singleEnd_g[$i]    ILLUMINACLIP:$Trimmomatic2_g:2:30:10:5   SLIDINGWINDOW:4:15  MINLEN:20      >> $output2_g/$temp.Trimmomatic.runLog.txt  2>&1");                    
        system("trim_galore  --quality 20   --phred33  --length 30      --clip_R1 1        --three_prime_clip_R1 0         --output_dir  $output_g       $output_g/Trimmomatic/$singleEnd_g[$i]      >> $output2_g/$temp.trim_galore.runLog  2>&1");   
