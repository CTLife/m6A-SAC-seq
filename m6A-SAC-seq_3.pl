...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2-removedAdapters"   
my $output_g = '';  ## such as "3-removedDups"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        Step 3: Remove PCR and optical duplicates by using clumpify.sh of BBMap. 
                ...
                The versions of softwares and packages are used in this script (This is shown for reference only. And do not need to match them.): 
                        ...
                        BBMap,         38.73  (clumpify.sh --version) 
                        MultiQC,       1.8    (multiqc --version)
       
        Usage:
               m6A-SAC-seq_3.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               m6A-SAC-seq_3.pl   -in 2-removedAdapters   -out 3-removedDups    > m6A-SAC-seq_3.runLog.txt  2>&1   
...
## Version Infromation
my $version = "    The 3rd Step, version 1.0.0,  2020-03-01.";
...
$input_g  = '2-removedAdapters';   ## This is only an initialization value or suggesting value, not default value.
$output_g = '3-removedDups';       ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'm6A-SAC-seq_3.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("clumpify.sh   in=$input_g/$end1    in2=$input_g/$end2    out=$output_g/$end1  out2=$output_g/$end2   reorder=f  dedupe=t  addcount=t  subs=0  >> $output2_g/$temp.runLog  2>&1");                                                   
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("clumpify.sh    in=$input_g/$singleEnd_g[$i]    out=$output_g/$singleEnd_g[$i]   reorder=f  dedupe=t  addcount=t   subs=0  >> $output2_g/$temp.runLog.txt  2>&1");                    
}
}
###################################################################################################################################################################################################
