...
my $output_g = '';  ## such as "4-finalFASTQ"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Further trim reads by using trim_galore. 
                ...
                The versions of softwares and packages are used in this script (This is shown for reference only. And do not need to match them.): 
                        ...
                        trim_galore,   0.6.4   (prinseq-lite.pl  -version) 
                        MultiQC,       1.8     (multiqc --version)
       
        Usage:
               m6A-SAC-seq_4.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               m6A-SAC-seq_4.pl   -in 3-removedDups   -out 4-finalFASTQ    > m6A-SAC-seq_4.runLog.txt  2>&1   
...
$output_g = '4-finalFASTQ';       ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'm6A-SAC-seq_4.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq$//    ;
        $temp =~ s/\.R1$//  or   die  ;
        system("trim_galore  --quality 20   --phred33  --length 15   --clip_R1 5     --clip_R2 11      --three_prime_clip_R1  11     --three_prime_clip_R2  5    --output_dir  $output2_g/trim_galore    --paired  $input_g/$end1  $input_g/$end2          >> $output2_g/trim_galore/$temp.runLog  2>&1"); 
        my $end1a = $end1;
        my $end2a = $end2;     
        $end1a =~ s/\.R1\.fastq/.R1_val_1.fq/  or die;   
        $end2a =~ s/\.R2\.fastq/.R2_val_2.fq/  or die;   
        system("mv  $output2_g/trim_galore/$end1a  $output_g/$end1");
        system("mv  $output2_g/trim_galore/$end2a  $output_g/$end2");
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("trim_galore  --quality 20   --phred33  --length 25    --clip_R1 5    --three_prime_clip_R1  11   --output_dir  $output2_g/trim_galore       $input_g/$singleEnd_g[$i]      >> $output2_g/trim_galore/$temp.runLog  2>&1");   
        my $end1a = $singleEnd_g[$i];
        $end1a =~ s/\.fastq/_trimmed.fq/  or die;   
        system("mv  $output2_g/trim_galore/$end1a  $output_g/$singleEnd_g[$i]");
}
}
###################################################################################################################################################################################################
