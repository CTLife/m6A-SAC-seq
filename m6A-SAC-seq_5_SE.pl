...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 5: Further trim reads by using bbduk.sh of BBMap. 
                ...
                The versions of softwares and packages are used in this script (This is shown for reference only. And do not need to match them.): 
                        ...
                        bbduk.sh,      38.76   (bbduk.sh  -v) 
                        MultiQC,       1.8     (multiqc --version)
       
        Usage:
               m6A-SAC-seq_5_SE.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               m6A-SAC-seq_5_SE.pl   -in 4-removedBarcodes   -out 5-finalFASTQ    > m6A-SAC-seq_5_SE.runLog.txt  2>&1   
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'm6A-SAC-seq_5_SE.pl  -help' \n";
    exit 0;
}
...
{
say    "\n\n\n\n\n\n##################################################################################################";
say    "Trim reads by using bbduk.sh of BBMap ......";          
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("bbduk.sh  in=$input_g/$end1    in2=$input_g/$end2    out=$output_g/$end1  out2=$output_g/$end2  ref=$Trimmomatic_g  stats=$output2_g/$temp.stats  statscolumns=5  minlength=10  ktrim=l  k=12  hdist=0     >> $output2_g/$temp.runLog  2>&1");     ##mink=11                                                
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("bbduk.sh  in=$input_g/$singleEnd_g[$i]    out=$output_g/$singleEnd_g[$i]   ref=$Trimmomatic_g  stats=$output2_g/$temp.stats   statscolumns=5  minlength=12  ktrim=l  k=12    hdist=0    >> $output2_g/$temp.runLog.txt  2>&1");     ##mink=11               
}
}
###################################################################################################################################################################################################
