...
my $input_g    = '';  ## such as "5-rawBAM/3_STAR"
my $output_g   = '';  ## such as "6-finalBAM/3_STAR"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 6: Only the mapped reads with MAPQ>0 are retained and reads on other chromosomes, all of them are removed.
        ...
        Usage:
               perl m6A-SAC-seq_6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               perl m6A-SAC-seq_6.pl   -genome hg38   -in 5-rawBAM/3_STAR   -out 6-finalBAM/3_STAR       > m6A-SAC-seq_6.runLog
...
## Version Infromation
my $version = "    The 6th Step, version 1.4,  2022-08-05";
...
$input_g  = '5-rawBAM/3_STAR';           ## This is only an initialization value or suggesting value, not default value.
$output_g = '6-finalBAM/3_STAR';         ## This is only an initialization value or suggesting value, not default value.
...
sub myFilterSAM  {
    ...
    open(FILE4, ">", "$folder/num_mismatches_$filePre.txt")    or die "$!";  
    my $n1 = 0; ## all reads
    my $n2 = 0; ## kept reads
    ...
    while (my $line1=<FILE1>) {
        if ($line1 =~ m/^@/) {
            ...
            $line1 =~ m/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+/  or die;
            my $chr = $3;
            my $length1 = length($10);
            $n1++;
            if( ($chr =~ m/^chr[XYM\d]+/i)  and ($length1 > 10) ) {
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("samtools  view   -h    --threads $numCores_g    -q 0    -o $output_g/$temp.t.sam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  ");  
