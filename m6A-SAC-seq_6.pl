...
my $input_g    = '';  ## such as "5-finalFASTQ"
my $output_g   = '';  ## such as "6-rawBAM"
my $mismatch_g = '';  ## such as 0.3
...
my $HELP = '
        ...
        Usage:
               m6A-SAC-seq_6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               m6A-SAC-seq_6.pl   -genome hg38   -in 5-finalFASTQ   -out 6-rawBAM  -mis 0.3    > m6A-SAC-seq_6.runLog
...
## Version Infromation
my $version = "    The 6th Step, version 1.0.0,  2020-03-01";
...
$input_g  = '5-finalFASTQ';       ## This is only an initialization value or suggesting value, not default value.
$output_g = '6-rawBAM';           ## This is only an initialization value or suggesting value, not default value.
$mismatch_g = 0.3;                ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  m6A-SAC-seq_6.pl  -help' \n";
    exit 0;
}
...
my  $Subread_index_g   = "$commonPath_g/subread/RefGenome/$genome_g/$genome_g";
my  $GSNAP_index_g     = "RefGenome/$genome_g/$genome_g/$genome_g";
my  $Kallisto_index_g  = "$commonPath_g/kallisto/RefGenome/$genome_g/$genome_g.allRNA";
my  $Salmon_index_g    = "$commonPath_g/salmon/RefGenome/human/hg38_index";
my  $STAR_index_g      = "$commonPath_g/STAR/RefGenome/$genome_g";
my  $HISAT2_index_g    = "$commonPath_g/hisat2/RefGenome/$genome_g/$genome_g";
my  $STAR_ensembl_index_g      = "$commonPath_g/STAR/RefGenome_Gencode/$genome_g";
if($genome_g eq "mm10") {
      $Salmon_index_g    = "$commonPath_g/salmon/RefGenome/mouse/mm10_index";
}
###################################################################################################################################################################################################
...
###################################################################################################################################################################################################
my $Kallisto_1A_g   = "$output_g/1_Kallisto";
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("kallisto quant  --threads=$numCores_g       --index=$Kallisto_index_g    --output-dir=$Kallisto_1A_g/$temp    $input_g/$end1  $input_g/$end2    >> $Kallisto_1A_g/$temp.runLog  2>&1 ");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("kallisto quant  --threads=$numCores_g   --single    -l 250    -s 100    --index=$Kallisto_index_g    --output-dir=$Kallisto_1A_g/$temp    $input_g/$singleEnd_g[$i]    >> $Kallisto_1A_g/$temp.runLog  2>&1");
}
...
&myMakeDir("$Kallisto_1A_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $Kallisto_1A_g/QC_Results/MultiQC    --ignore QC_Results   $Kallisto_1A_g   >> $Kallisto_1A_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
...
###################################################################################################################################################################################################
my $Salmon_2A_g   = "$output_g/2_Salmon";
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("salmon quant  --threads $numCores_g   --libType OSR     --index $Salmon_index_g    --output $Salmon_2A_g/$temp    --mates1 $input_g/$end1  --mates2 $input_g/$end2    >>$Salmon_2A_g/$temp.runLog  2>&1 ");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("salmon quant  --threads $numCores_g    --libType U      --index $Salmon_index_g    --output $Salmon_2A_g/$temp    --unmatedReads $input_g/$singleEnd_g[$i]      >>$Salmon_2A_g/$temp.runLog  2>&1 ");
}
...
&myMakeDir("$Salmon_2A_g/QC_Results/MultiQC");
system( "multiqc    --title Aligner    --verbose  --export   --outdir $Salmon_2A_g/QC_Results/MultiQC    --ignore QC_Results   $Salmon_2A_g   >> $Salmon_2A_g/QC_Results/MultiQC/MultiQC.Aligner.runLog    2>&1" );     
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g    --quantMode   TranscriptomeSAM      --outSAMunmapped Within   --outSAMattributes All   --alignSJDBoverhangMin 1  --outFilterMismatchNoverReadLmax $mismatch_g  --outFilterMismatchNmax 999  --outFilterMultimapNmax 20  --outFileNamePrefix  $STAR_3A_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$end1  $input_g/$end2   >> $STAR_3A_g/$temp.runLog  2>&1 ");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("STAR  --runMode alignReads   --runThreadN $numCores_g    --quantMode   TranscriptomeSAM      --outSAMunmapped Within   --outSAMattributes All   --alignSJDBoverhangMin 1  --outFilterMismatchNoverReadLmax $mismatch_g    --outFilterMismatchNmax 999  --outFilterMultimapNmax 20     --outFileNamePrefix  $STAR_3A_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$singleEnd_g[$i]   >> $STAR_3A_g/$temp.runLog  2>&1 ");
        system("rename  s/Aligned.out.sam/sam/   $STAR_3A_g/*Aligned.out.sam");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g    --quantMode   TranscriptomeSAM      --outSAMunmapped Within   --outSAMattributes All   --alignSJDBoverhangMin 1  --outFilterMismatchNoverReadLmax $mismatch_g  --outFilterMismatchNmax 999  --outFilterMultimapNmax 20         --outFileNamePrefix  $STAR_3B_g/$temp.   --genomeDir $STAR_ensembl_index_g  --readFilesIn $inputDir2/$end1  $inputDir2/$end2   >>$STAR_3B_g/$temp.runLog  2>&1 ");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("STAR  --runMode alignReads   --runThreadN $numCores_g    --quantMode   TranscriptomeSAM      --outSAMunmapped Within   --outSAMattributes All   --alignSJDBoverhangMin 1  --outFilterMismatchNoverReadLmax $mismatch_g  --outFilterMismatchNmax 999  --outFilterMultimapNmax 20         --outFileNamePrefix  $STAR_3B_g/$temp.   --genomeDir $STAR_ensembl_index_g  --readFilesIn $inputDir2/$singleEnd_g[$i]   >>$STAR_3B_g/$temp.runLog  2>&1 ");
        system("rename  s/Aligned.out.sam/sam/   $STAR_3B_g/*Aligned.out.sam");
}
...
###################################################################################################################################################################################################
my $HISAT2_4A_g   = "$output_g/4_HISAT2";
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,  $end2\n";
        system("hisat2         --threads $numCores_g   -q   --phred33     --mp 2,1  --rna-strandness FR  -k 1  -x $HISAT2_index_g    -1 $input_g/$end1        -2 $input_g/$end2     -S $HISAT2_4A_g/$temp.sam    >>$HISAT2_4A_g/$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("hisat2        --threads $numCores_g   -q   --phred33     --mp 2,1  --rna-strandness FR  -k 1      -x $HISAT2_index_g    -U $input_g/$singleEnd_g[$i]        -S $HISAT2_4A_g/$temp.sam    >>$HISAT2_4A_g/$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,  $end2\n";
        system("subjunc  -T $numCores_g  -I 15  --multiMapping  -B 1  -M 30   --SAMoutput  -d 10  -D 800   -i $Subread_index_g   -r $input_g/$end1   -R  $input_g/$end2   -o  $subread_g/$temp.sam     >>$subread_g/$temp.runLog  2>&1");
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        my $temp = $singleEnd_g[$i];
        say   "\n\t......$temp\n";
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("subjunc  -T $numCores_g  -I 15  --multiMapping  -B 1  -M 30   --SAMoutput   -i $Subread_index_g    -r $input_g/$singleEnd_g[$i]    -o $subread_g/$temp.sam        >>$subread_g/$temp.runLog   2>&1");
}
} ## End subread
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,  $end2\n";
        system("gsnap    --db=$GSNAP_index_g    --nthreads=$numCores_g   --max-mismatches=$mismatch_g    --novelsplicing=1  --output-file=$GSNAP_g/$temp.sam   $input_g/$end1  $input_g/$end2    >>$GSNAP_g/$temp.runLog   2>&1");
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        my $temp = $singleEnd_g[$i];
        say   "\n\t......$temp\n";
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("gsnap    --db=$GSNAP_index_g    --nthreads=$numCores_g   --max-mismatches=$mismatch_g     --novelsplicing=1  --output-file=$GSNAP_g/$temp.sam       $input_g/$singleEnd_g[$i]    >>$GSNAP_g/$temp.runLog   2>&1");
}
}  ########## End GSNAP
&myQC_BAM_1($GSNAP_g);
###################################################################################################################################################################################################
